...
public class WorldBuilder implements BoardAPI {
	...
	private static NamedLocation[] squares = new NamedLocation[NUM_SQUARES];
	private Vector<String> all_locations = new Vector<String>();
	
	private Domain brown = new Domain("brown");
	private Domain lightBlue = new Domain("dark blue");
	...
	private Domain darkBlue = new Domain("light blue");
	...
	public WorldBuilder(Dice dice){
		...
		squares[2] = new CommunityChest("Community", null, squares[1]);		
		...
		squares[5] = new Vehicle(characters.getRandomVehicle(), 200, 100,new int[] {25,50,100,200}, null, squares[4]);
		
		
		
		
		domain = domains.getDomain(1);
		locations = characters.getRandomLocationsFromDomain(domain, 3, all_locations);//Subsequent color groups have 3 properties
		updateLocations(locations);
		squares[6] = new InvestmentProperty(locations.elementAt(0), 100, 50,new int[] {6,30,90,270,400,550}, null, squares[5], lightBlue, 50);
		squares[7] = new Chance("Chance", null, null);
		...
		locations = characters.getRandomLocationsFromDomain(domain, 3, all_locations);
		updateLocations(locations);
		...
		squares[15] = new Vehicle(characters.getRandomVehicle(), 200, 100,new int[] {25,50,100,200}, null, squares[14]);
		...
		squares[17] =  new CommunityChest("Community", null, null);
		...
		squares[22] = new Chance("Chance", null, null);
		...
		squares[25] = new Vehicle(characters.getRandomVehicle(), 200, 100,new int[] {25,50,100,200},null, squares[24]);
		...
		squares[30] = new GoToJail("Go To Jail", null, null);
		...
		squares[33] =  new CommunityChest("Community", null, null);
		...
		squares[35] = new Vehicle(characters.getRandomVehicle(), 200, 100,new int[] {25,50,100,200},null, squares[34]);
		squares[36] = new Chance("Chance", null, null);
		squares[36].setRight(squares[35]);

		
		
		domain = domains.getDomain(7);
		locations = characters.getRandomLocationsFromDomain(domain, 2, all_locations);//Last colour group only has two properties
	...
	private void updateLocations(Vector<String> locations){
		
		for(String location : locations){
			all_locations.addElement(location);
		}
...
//	}
	...
	public String getCharacter(int number){
		...
		String character = characters.getRandomPersonFromDomain(domain).getName();
		return character;
	}
	...
	public static Vector<InvestmentProperty>getSites(){
		Vector<InvestmentProperty> sites = new Vector<InvestmentProperty>();
		
		for(NamedLocation s : squares){
			if(s instanceof InvestmentProperty){
				sites.add((InvestmentProperty) s);
			}
		}
	...
	public boolean isSite(String name) {
		...
		for (NamedLocation s :squares) {
			if (s instanceof InvestmentProperty) {
				InvestmentProperty p = (InvestmentProperty) s;
				if (p.equals(name)) {	
					found = true;
				}
			}
